...
object Huffman {
  ...
  def convert(tree: CodeTree): CodeTable = {
    def traverse(subTree: CodeTree, bitAcc: List[Bit]): CodeTable = subTree match {
      case Leaf(char, _) => List((char, bitAcc))
      case Fork(left, right, _, _) => traverse(left, bitAcc ::: List(0)) ::: traverse(right, bitAcc ::: List(1))
    }
