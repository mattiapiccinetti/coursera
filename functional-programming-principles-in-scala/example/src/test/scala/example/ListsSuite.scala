 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a more than three numbers") {
  ...
  test("sum of many zeros") {
    assert(sum(List(0, 0, 0)) === 0)
  }

  test("sum of repeated mixed numberd") {
    assert(sum(List(0, -1, 0, 3, 7, 1, -3)) === 7)
  }
  ...
  test("max of more than three numbers") {
    assert(max(List(13, 19, 45, -3)) === 45)
  }

  test("max of negative numbers") {
    assert(max(List(-2, -1, -7)) === -1)
  }

  test("max of negative and positive numbers") {
    assert(max(List(-2, 4, -7)) === 4)
  }
