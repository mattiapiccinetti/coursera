...
object Anagrams {
  ...
  object Occurrence { def apply(char: Char, freq: Int): Occurrence = (char, freq) }
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(identity).map { case (k, v) => Occurrence(k, v.size) }.toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = (x ::: y)
    .groupBy { _._1 }
    .map { case (k, v) => v.reduce { (a, b) => (a._1, Math.abs(a._2 - b._2)) } }
    .filter { case (k, v) => v != 0 }
    .toList
    .sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def anagramify(occurrences: Occurrences): List[Sentence] = occurrences match {
      case Nil => List(Nil)
      case _ => for {
        combo <- combinations(occurrences)
        word <- dictionaryByOccurrences(combo)
        tail <- anagramify(subtract(occurrences, combo))
      } yield word :: tail
    }
    anagramify(sentenceOccurrences(sentence))
  }
}
